...
/*Read one byte of data from peripheral registry
* spi: the spi connected to that peripheral
* reg: the address of memmory (of the per.) that we want to read
*/
uint8_t read8(Spi *spi, uint8_t reg);

/*Write one byte of data into peripheral registry
* spi: the spi connected to that peripheral
* reg: the address of memmory (of the per.) that we want to overwrite
* value: the value to be written in reg
*/
void write8(Spi *spi,
            uint8_t reg,
            uint8_t value);
