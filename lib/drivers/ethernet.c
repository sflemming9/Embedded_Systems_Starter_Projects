...
bool ETH_InitSmiGpio(Smi *smi) {
  ...
  Pin_ConfigGpioPin(&(smi->pin_mdio), GPIO_Mode_AF, GPIO_Speed_100MHz,
                    GPIO_OType_PP, GPIO_PuPd_NOPULL, true);
  Pin_ConfigGpioPin(&(smi->pin_mdc), GPIO_Mode_AF, GPIO_Speed_100MHz,
                    GPIO_OType_PP, GPIO_PuPd_NOPULL, true);

  // Pick the SMI clock divisor.
  RCC_ClocksTypeDef RCC_ClocksStatus;
  RCC_GetClocksFreq(&RCC_ClocksStatus);
  uint32_t hclk_freq = RCC_ClocksStatus.HCLK_Frequency;
  uint32_t clk_div_bits = 0;
  
  if(hclk_freq >= (150 * 1000 * 1000)) {
    clk_div_bits = ETH_MACMIIAR_CR_Div102;
  } else if (hclk_freq >= (100 * 1000 * 1000)) {
    clk_div_bits = ETH_MACMIIAR_CR_Div62;
  } else if (hclk_freq >= (60 * 1000 * 1000)) {
    clk_div_bits = ETH_MACMIIAR_CR_Div42;
  } else if (hclk_freq >= (35 * 1000 * 1000)) {
...
bool ETH_InitMiiGpio(Mii *mii) {
  ...
  for(int a = 0; a < 15; a++) {
    if(&pins[a]) {
      Pin_ConfigGpioPin(&pins[a], GPIO_Mode_AF, GPIO_Speed_100MHz,
                        GPIO_OType_PP, GPIO_PuPd_NOPULL, true);
    }
  }
...
void ETH_MacInit(void) {
  ...
  maccr |=
      (1u << 25) |  // Enable FCS stripping on frames sent to software.
